<div class="bg-white rounded-lg lg:rounded-2xl overflow-hidden border border-border/50 shadow-sm hover:shadow-premium group transition-all duration-300 relative flex flex-col h-full">
    ...
    @if($product->stock_quantity > 0 && $product->stock_quantity <= 5)
        <div class="absolute bottom-0 left-0 right-0 bg-linear-to-r from-orange-500 to-amber-500 text-white text-[0.68rem] font-black text-center py-1 px-2 z-10 tracking-wider">
    ...
    <div class="p-3 lg:p-4 flex flex-col flex-1">
        <div class="text-[0.65rem] lg:text-[0.72rem] font-black text-light uppercase tracking-widest mb-1">{{ $product->brand }}</div>
        <h3 class="text-[0.82rem] lg:text-[0.92rem] font-bold text-dark leading-tight line-clamp-2 min-h-[2.4rem] lg:min-h-[2.6rem] mb-1.5 group-hover:text-primary transition-colors">
        ...
        <div class="mt-auto pt-2">
            <div class="flex items-baseline flex-wrap gap-1.5 mb-3">
                <span class="text-base lg:text-lg font-black text-dark tracking-tight">{{ number_format($product->price, 2) }} <span class="text-[0.6em]">MAD</span></span>
                @if($product->old_price)
                    <span class="text-xs lg:text-sm text-light line-through decoration-danger/40">{{ number_format($product->old_price, 2) }}</span>
                @endif
            </div>
            ...
            <div class="flex items-center gap-2">
                @if($product->stock_quantity > 0)
                    <form action="{{ route('cart.add') }}" method="POST" class="flex-1">
                        ...
                        <input type="hidden" name="product_id" value="{{ $product->id }}">
                        <input type="hidden" name="quantity" value="1">
                        <button type="submit" class="w-full py-2 bg-primary text-white text-[0.72rem] lg:text-[0.82rem] font-black rounded-lg hover:bg-primary-dark transition-all shadow-md active:scale-95 flex items-center justify-center gap-2 whitespace-nowrap overflow-hidden text-ellipsis">
                            🛒 <span class="shrink-0">Add to Cart</span>
                        </button>
                    </form>
                @else
                    <button class="flex-1 py-2 bg-surface text-light text-[0.72rem] lg:text-[0.82rem] font-bold rounded-lg cursor-not-allowed" disabled>Out of Stock</button>
                @endif

                <a href="{{ route('products.show', [$product->id, $product->slug]) }}" class="w-9 h-9 lg:w-10 lg:h-10 shrink-0 flex items-center justify-center rounded-lg bg-surface border border-border/50 text-dark hover:bg-white hover:text-primary hover:border-primary transition-all active:scale-90" title="View Details">
